...
func Decoder(bytes []byte) (*binary.ClassFile, bool) {
	...
	cf.Magic = utils.BigEndian2Little4U4((bytes)[index : index+4])
	index += 4
	cf.MinorVersion = utils.BigEndian2Little4U2((bytes)[index : index+2])
	index += 2
	cf.MajorVersion = utils.BigEndian2Little4U2((bytes)[index : index+2])
	index += 2

	cf.ConstantPoolCount = utils.BigEndian2Little4U2((bytes)[index : index+2])
	index += 2
	offset, _ := ConstantPool(bytes[index:], &cf)
	index = index + offset

	cf.AccessFlags = binary.AccessFlags(utils.BigEndian2Little4U2((bytes)[index : index+2]))
	index += 2
	cf.ThisClass = utils.BigEndian2Little4U2((bytes)[index : index+2])
	index += 2
	cf.SuperClass = utils.BigEndian2Little4U2((bytes)[index : index+2])
	index += 2

	cf.InterfacesCount = utils.BigEndian2Little4U2((bytes)[index : index+2])
	index += 2
	offset, _ = Interface(bytes[index:], &cf)
	index = index + offset

	cf.FieldsCount = utils.BigEndian2Little4U2((bytes)[index : index+2])
	index += 2
	offset, _ = Field(bytes[index:], &cf)
	index = index + offset

	cf.MethodsCount = utils.BigEndian2Little4U2((bytes)[index : index+2])
	index += 2
	offset, _ = Method(bytes[index:], &cf)
	index = index + offset

	cf.AttributesCount = utils.BigEndian2Little4U2((bytes)[index : index+2])
	index += 2
