...
func init()  {
	INSTRUCTION_MAP[0x86] = &I_i2f{}
}

func (s I_i2f)Stroke(ctx *runtime.Context) error {
	utils.Log(1, "i2f exce >>>>>>>>>\n")
	...
	ctx.CurrentFrame.PushFrame(types.Jfloat(value.(types.Jint)))
	return nil
}

func (s I_i2f)Test(octx *runtime.Context) *runtime.Context {
	f := new(runtime.Frame)
...
======================================================================================
		...
		描述				||		value 必须是在操作数栈栈顶的 int 类型数据，指令执行时，它将从操作数 栈中出栈，
						||		使用IEEE 754规范的向最接近数舍入模式转换成float类型数据，然后压入到操作数栈之中。
						||
======================================================================================
						||		
						||
