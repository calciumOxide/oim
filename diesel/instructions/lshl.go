...
func init()  {
	INSTRUCTION_MAP[0x78] = &I_ishl{}
}

func (s I_ishl)Stroke(ctx *runtime.Context) error {
	utils.Log(1, "ishl exce >>>>>>>>>\n")
...
func (s I_ishl)Test(octx *runtime.Context) *runtime.Context {
	f := new(runtime.Frame)
...
======================================================================================
						||
						||		value1 和 value2 都必须为 int 类型数据，指令执行时，value1 和 value2 从操作数栈中出栈，然后将 value1 左移 s 位，
...
======================================================================================
						||
						||
						||这个操作(即使是出现了溢出的情况下)等同于把 value1 乘以 2 的 s 次方， 位移的距离实际上被限制在 0 到 31 之间，相当于指令执行时会把 value2 与0x1f 做一遍算术与操作。
		注意				||
						||
						||
						||
