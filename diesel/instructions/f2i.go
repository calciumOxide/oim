...
func (s I_f2i)Stroke(ctx *runtime.Context) error {
	...
	ctx.CurrentFrame.PushFrame(types.Jint(value.(types.Jfloat)))
	return nil
}

func (s I_f2i)Test(octx *runtime.Context) *runtime.Context {
	f := new(runtime.Frame)
...
======================================================================================
						...
						||		再转换为 int 类型值 result。然后 result 被压入到操作数栈中。
						||
...
======================================================================================
						||
						||
						||		f2i 指令执行了窄化类型转换(Narrowing Primitive Conversion，JLS3 §5.1.3)，它可能会导致 value’的数值大小和精度发生丢失。
		注意				||
						||
						||
						||
