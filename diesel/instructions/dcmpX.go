...
func (s I_dcmpX)Stroke(ctx *runtime.Context) error {
	...
	if reflect.TypeOf(value1) == reflect.TypeOf(types.JDO) {
		value1 = types.Jdouble(types.JDO)
	}
	if reflect.TypeOf(value2) == reflect.TypeOf(types.JDO) {
		value2 = types.Jdouble(types.JDO)
	}
	...
	if value1.(types.Jdouble) > value2.(types.Jdouble) {
		ctx.CurrentFrame.PushFrame(types.Jint(1))
	} else if value1.(types.Jdouble) < value2.(types.Jdouble) {
		ctx.CurrentFrame.PushFrame(types.Jint(-1))
	} else {
...
func (s I_dcmpX)Test(octx *runtime.Context) *runtime.Context {
	f := new(runtime.Frame)
