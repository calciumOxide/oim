...
func init()  {
	INSTRUCTION_MAP[0x6f] = &I_ddiv{}
}

func (s I_ddiv)Stroke(ctx *runtime.Context) error {
	utils.Log(1, "ddiv exce >>>>>>>>>\n")
	...
	if value2 == reflect.TypeOf(types.JDU) || value2 == reflect.TypeOf(types.JDO) {
		ctx.CurrentFrame.PushFrame(types.Jdouble(0))
		return nil
	}
	...
	if value1.(types.Jdouble) == 0.0 && value2.(types.Jdouble) == 0.0 {
		ctx.CurrentFrame.PushFrame(types.JDN)
		return nil
	}
...
======================================================================================
						...
						||		接着将这两个数值相除(value1’÷value2’)，结 果转换为 double 类型值 result，最后 result 被压入到操作数栈中。
						||
		描述				||		ddiv 指令的运算结果取决于 IEEE 规范中规定的运算规则:
						...
						||				并以IEEE 754规范的最接近数舍入模式得到运算结果， 如果运算结果的绝对值太大以至于无法使用 double 类型来表示，换句话说就是出现了上限溢出，
						||				那结果将会使用具有适当符号的无穷大来代替。
						||				如果运算结果的绝对值太小以至于无法使用 double 类型来表示，换句话 说就是出现了下限溢出，那结果将会使用具有适当符号的零值来代替。
						||
 						||		Java 虚拟机必须支持 IEEE 754 中定义的逐级下溢(Gradual Underflow)， 尽管指令执行期间，上溢、下溢以及精度丢失等情况都有可能发生，但 ddiv 指令永远不会抛出任何运行时异常。
						||
======================================================================================
						||		
						||
